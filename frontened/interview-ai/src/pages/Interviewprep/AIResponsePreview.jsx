            ...
            components={{
                code({node,className,children,...props}){
                    ...
                    return !isInline ?(
                        ...
                        language={language}></CodeBlock>
                    ): (
                         <code className='px-1 py-0,5 bg-amber-100 rounded text-sm' {...props}>
                            {children}
                         </code>
                    );
                },
...
function CodeBlock({code,language}){
    ...
    return <div className='relative my-6 rounded-lg overflow-x-hidden bg-gray-50 border border-gray-200'>
           <div className='flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200'>
             ...
             <button onClick={copyCode} className='text-gray-500 hover:text-gray-700 focus:outline-none relative' aria-label="Copy code">
                   {copied ? (<LuCheck size={16} className='text-green-600'></LuCheck>):(
                        <LuCopy size={16}/>
                      )}
                   {copied  && (
                     <span className='absolute -top-8 right-0 bg-black text-white text-xs rounderd-md px-2 py-1 opacity-80 group-hover:opacity-100 transition'>
                        Copied!
                     </span>
                   )}
             </button>
